...
def _simple_yaml_load(text):
    """Minimal YAML parser — imported pattern from techniques.py."""
    result = {}
    current_list = None
        ...
        if stripped.startswith('- '):
            if current_list is not None:
                ...
                if ':' in val and not val.startswith('"'):
                    k, _, v = val.partition(':')
                    current_list.append({k.strip(): v.strip().strip('"').strip("'")})
                else:
...
def get_project_root() -> Path:
    ...
    # Default: assume we're somewhere in the project
        # hooks/autonomous-stan/lib/ → 4 levels up to repo root
